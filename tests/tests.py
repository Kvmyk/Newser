...
async def test_fetch_news_with_topic():
    ...
    mock_response.json.return_value = {
        'results': [
            {'title': 'Nowa polityka zagraniczna Polski wobec krajów UE', 'link': 'https://www.gazeta.pl/wiadomosci/7,114884,30675821,polityka-zagraniczna.html'},
            {'title': 'Wzrost inflacji w Polsce - eksperci alarmują', 'link': 'https://www.onet.pl/informacje/onetwiadomosci/gospodarka-wzrost-inflacji/qm6j3kl'},
            {'title': 'Minister Sportu o planach rozwoju infrastruktury na Euro 2028', 'link': 'https://www.rmf24.pl/tylko-w-rmf24/wywiady/news-minister-sportu-o-planach-na-przyszlosc,nId,7312654'}
        ]
    }
...
async def test_add_favorite():
    ...
    last_articles[str(ctx.author.id)] = [
        {'title': 'PKB Polski wzrósł o 3,5% w pierwszym kwartale 2023', 'link': 'https://www.money.pl/gospodarka/pkb-polski-wzrost-2023,263,0,2526789.html'},
        {'title': 'Kolejne rozmowy pokojowe zakończone fiaskiem', 'link': 'https://www.tvn24.pl/wiadomosci-ze-swiata,2/kolejne-rozmowy-pokojowe,1280564.html'}
    ]
    ...
    assert favorites[str(ctx.author.id)][0]['title'] == 'PKB Polski wzrósł o 3,5% w pierwszym kwartale 2023'
    ctx.send.assert_called_once()
...
async def test_add_favorite_invalid_index():
    ...
    last_articles[str(ctx.author.id)] = [
        {'title': 'Nowe inwestycje w przemyśle - 2 mld zł na rozwój sektora motoryzacyjnego', 'link': 'https://www.interia.pl/biznes/news-nowe-inwestycje-w-przemysle,nId,7098442'}
    ]
...
async def test_handle_favorites_with_items():
    ...
    from src.newser import favorites
    favorites[str(ctx.author.id)] = [
        {'title': 'Nowy projekt ustawy o ochronie środowiska - co się zmieni?', 'link': 'https://www.rp.pl/polityka/art39087981-nowy-projekt-ustawy'},
        {'title': 'Reforma edukacji 2023 - najważniejsze zmiany dla uczniów i nauczycieli', 'link': 'https://www.newsweek.pl/polska/spoleczenstwo/reforma-edukacji-2023/4n8xjlp'}
    ]
...
async def test_remove_favorite():
    ...
    from src.newser import favorites
    favorites[str(ctx.author.id)] = [
        {'title': 'Rozwój sztucznej inteligencji w Polsce - raport ministerstwa cyfryzacji', 'link': 'https://www.pb.pl/technologie/rozwoj-sztucznej-inteligencji-w-polsce,1548321'},
        {'title': 'Nowe odkrycie polskich archeologów - sensacyjne znalezisko sprzed 3000 lat', 'link': 'https://www.naukawpolsce.pl/aktualnosci/news,96325,nowe-odkrycie-polskich-archeologow.html'}
    ]
    ...
    assert len(favorites[str(ctx.author.id)]) == 1
    assert favorites[str(ctx.author.id)][0]['title'] == 'Nowe odkrycie polskich archeologów - sensacyjne znalezisko sprzed 3000 lat'
    ctx.send.assert_called_once()
...
async def test_remove_favorite_invalid_index():
    ...
    from src.newser import favorites
    favorites[str(ctx.author.id)] = [
        {'title': 'Raport ekonomiczny 2023 - zdrowie polskiej gospodarki', 'link': 'https://www.pap.pl/aktualnosci/raport-ekonomiczny-2023-zdrowie-gospodarki'}
    ]
